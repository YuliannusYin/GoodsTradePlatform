...
import type { Product, EditProductDto } from '@/types/product';
...
export default defineComponent({
  ...
  setup() {
    ...
    const editForm = ref<EditProductDto>({
      name: '',
      description: '',
      imageUrls: [],
      price: 0,
    ...
    async function loadProducts() {
      ...
      error.value = null;
      try {
        products.value = await userProductStore.API.getMyProducts();
      } catch (e: any) {
        error.value = '加载商品失败';
      } finally {
        loading.value = false;
    ...
    async function handleEditSubmit() {
      ...
      try {
        ...
        await userProductStore.API.editProduct(editingProductId.value, editForm.value);
        closeEditDialog();
        await loadProducts();
      } catch (e: any) {
        dialogError.value = e?.data?.message || '编辑失败';
      } finally {
    ...
    async function handleResubmit(product: Product) {
      ...
      try {
        ...
        await userProductStore.API.editProduct(product.id, dto);
        await loadProducts();
      } catch (e: any) {
        error.value = e?.data?.message || '重新提交失败';
      }
    }
    ...
    async function handleDelete(product: Product) {
      ...
      try {
        await userProductStore.API.deleteProduct(product.id);
        await loadProducts();
      } catch (e: any) {
    ...
    return {
      ...
      showEditDialog, editForm, editImageUrlsText, submitting, dialogError,
      PRODUCT_CATEGORIES, PRODUCT_CONDITIONS, PRODUCT_STATUSES, PRODUCT_STATUS_COLORS,
      openEditDialog, closeEditDialog, handleEditSubmit, handleResubmit, handleDelete
    };
  }
});
</script>
